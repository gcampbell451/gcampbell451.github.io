...
# boxplot for wind v. ozone, per our discussion
# round wind column, use myairquality set because there are no NAs
myairquality$roundWind <- round(myairquality$Wind)
windBox <- ggplot(myairquality, aes(x=as.factor(roundWind), y=Ozone))
...
library(reshape2)
myairqualityClean$Date <- as.Date(paste("1973", myairqualityClean$Month, myairqualityClean$Day, sep = "-"))
# use melt function to stack set of columns into single column of data
melted <- melt(myairqualityClean, id.vars = c("Date"))
heatmap <- ggplot(melted, aes(x = Date, y= variable, fill = value)) + geom_tile()
heatmap

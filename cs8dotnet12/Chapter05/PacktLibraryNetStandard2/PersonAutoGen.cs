...
public partial class Person
{
    ...
    // two readonly properties defined using C# 6 or later lambda expression syntax
    ...
    public WondersOfTheAncientWorld FavoriteAncientWonder
    {
        ...
        set
        {
            ...
            {
                throw new ArgumentException(
                    "Favorite ancient wonder can only have a single enum value.", 
                    nameof(FavoriteAncientWonder));
            }
            if (!Enum.IsDefined(typeof(WondersOfTheAncientWorld), value))
            {
                throw new ArgumentException(
                    $"{value} is not a member of the WondersOfheAncientWorld enum.",
                    nameof(FavoriteAncientWonder));
            }
    ...
    #region Indexers: Preoerties that use array syntax to access them
